...
app.post('/signup', (req, res) => {
  ...
  let validationUser = registerService.validate(info);

  if (validationUser.email === null) {
    let registration = registerService.authenticate(info);
...
app.post('/login', (req, res) => {
  ...
  // validate inputs for white space
  let validInputs = loginService.validate(credentials);
  ...
  if (validInputs.email == null) {
    ...
    res.render('login', {
      emailWarning: validInputs.email,
      passwordWarning: validInputs.password,
      email: req.body.email,
      password: req.body.password
    });
  }
